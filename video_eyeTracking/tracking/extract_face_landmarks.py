...
def extract_face_landmarks(video_path, iris_distance=200, min_detection_confidence=0.8, min_tracking_confidence=0.8, output_path=None):
    ...
    if output_path:
        ...
        if os.path.exists(output_video_path):
            os.remove(output_video_path)  # Remove the existing file to avoid conflicts
    ...
    with tqdm(total=num_frames, desc="Extracting face landmarks") as pbar:
        for thisFrame in range(num_frames):
            ...
            if results.multi_face_landmarks:
                ...
                # Gaze estimation 
                absGaze_rightEye, absGaze_leftEye, PoG, vergence, head_pose, relGaze_rightEye, relGaze_leftEye = gaze(frame, results.multi_face_landmarks[0], iris_distance=iris_distance)
                ...
                gaze_log_entry = [
                    ...
                    absGaze_rightEye[0], absGaze_rightEye[1], absGaze_leftEye[0], absGaze_leftEye[1],
                    PoG[0], PoG[1], vergence, head_pose[0], head_pose[1],
                    relGaze_rightEye[0], relGaze_rightEye[1], relGaze_leftEye[0], relGaze_leftEye[1]
                ]
            ...
            if output_path:
                try:
                    ...
                    print(f"Error writing frame {thisFrame + 1} to video: {e}")
                    break
